...
做完 MVP （Model，View，Projection）变换后，就得到了一个正则化的（canonical） $[-1,1]^3$ 的正方体空间。之后便是要将正方体空间中的物体正确的投影到屏幕上。而这一步就叫做光栅化（Rasterize）。
...
* 定义三角形的三个定点属性即可插值出三角形内的所有点属性（质心插值算法)
...
> 推导见 [重心坐标系推导](https://blog.ashechol.top/2023/01/06/computer%20graphic/%E9%87%8D%E5%BF%83%E5%9D%90%E6%A0%87%E7%B3%BB/)

现今的 GPU 有专门的硬件用于处理点在三角形内外的判断。
...
2. 依次绘制每个三角形的像素，且在绘制时采取一下方法
   * 如果该像素的 z 值小于，则绘制该像素，同时更新深度缓冲对应像素的深度值
   * 否则，跳过该像素（说明已经有像素覆盖该像素了）
